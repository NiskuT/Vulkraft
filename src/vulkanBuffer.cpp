...
vulkanBuffer::~vulkanBuffer() {
  unmap();
  vkDestroyBuffer(engineDev.device(), buffer, nullptr);
  vkFreeMemory(engineDev.device(), memory, nullptr);
}
...
VkResult vulkanBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
  ...
  return vkMapMemory(engineDev.device(), memory, offset, size, 0, &mapped);
}
...
void vulkanBuffer::unmap() {
  if (mapped) {
    vkUnmapMemory(engineDev.device(), memory);
    mapped = nullptr;
  }
}
...
VkResult vulkanBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
  ...
  return vkFlushMappedMemoryRanges(engineDev.device(), 1, &mappedRange);
}
...
VkResult vulkanBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
  ...
  mappedRange.offset = offset;
  mappedRange.size = size;
  return vkInvalidateMappedMemoryRanges(engineDev.device(), 1, &mappedRange);
}
